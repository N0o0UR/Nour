...
### Performance tag is equally influenced by no_of_times_90_dpd_or_worse_in_last_6_months which ranges from 7 to 125 months
...
## Building Random Forest Model for Demographic Data
    
```{r ,warning=FALSE , message=FALSE,echo=FALSE,results=FALSE}

set.seed(120) # Setting seed
classifier_RF_1 = randomForest(x = train_data_balanced_over, y = train_data_balanced_over$performance_tag, ntree = 25, importance = TRUE)

print(classifier_RF_1)

# Predicting the Test set results
y_pred_1 = predict(classifier_RF_1, newdata = woe_demogs_testing_data)

# Confusion Matrix
confusion_mtx_1 = table(actual_values_1, y_pred_1)
print(confusion_mtx_1)

cat("The specificity of Random Forest model with test dataset of demographic data is ", specificity(confusion_mtx_1)*100,"%","\n")

cat("The sensitivity of Random Forest model with test dataset of demographic data is ", sensitivity(confusion_mtx_1)*100,"%","\n","\n")
...
Building Random Forest Model using both demographics and credit bureau datasets

```{r ,warning=FALSE , message=FALSE,echo=FALSE,results=FALSE}

set.seed(120) # Setting seed
classifier_RF_2 = randomForest(x = train_data_balanced_over_both, y = train_data_balanced_over_both$performance_tag, ntree = 25,importance = TRUE)
print(classifier_RF_2)

# Predicting the Test set results
y_pred_2 = predict(classifier_RF_2, newdata = woe_both_testing_data)

# Confusion Matrix
confusion_mtx_2 = table(actual_values_3, y_pred_2)
print(confusion_mtx_2)

# Specificity and Sensitivity of Random Forest Model

cat("The specificity of Random Forest model with test dataset of demographic data is ", specificity(confusion_mtx_2)*100,"%","\n")

cat("The sensitivity of Random Forest model with test dataset of demographic data is ", sensitivity(confusion_mtx_2)*100,"%","\n","\n")

# Plotting model
plot(classifier_RF_2)
