...
summary: ''
authors: []
lastmod: '2021-07-17T11:51:24-03:00'
featured: no
image:
  caption: ''
  focal_point: ''
...
## Evaluating our multiple Logistic regression Model
...
After passing the test data set that represent 25% of the original data set to the developed multiple Logistic regression Model and evaluating the classification output with reference to the original output we got accuracy of 90.51%.
...
```
## Updating the model with the significant parameters only


 In this part we will update our model to include only the significant predictors in the process of classification.
...
updated_mdl<- update (full_mdl , ~ job+ education +housing+ loan+contact +month+duration+campaign+poutcome)
summary(updated_mdl)
```
## Evaluating the updated Model


After passing the test data set to the updated model and evaluating the classification output with respect to the observed values we got accuracy of 90.54% which is higher than the accuracy that we got by the model which includes all the predictors. 
...
p_updated<- predict(updated_mdl, explanatory_data,type = "response")
summary(p)

predicted_response_updated<-ifelse(p_updated>0.5,"yes","no")

outcomes_updated<- table(predicted_response_updated,test$y)

confusion_updated <- conf_mat(outcomes_updated)

autoplot(confusion_updated)

summary(confusion_updated, event_level = "second")
...
By comparing the accuracy associated with the first model that used all the predictors to the second model that used only the significant predictors, The accuracy associated with the second model is considered higher than the first one and the second model is considered less computationally expensive than the first model.
